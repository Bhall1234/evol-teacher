...
def diversify_questions(instructions, model: str, temperature: float, max_tokens: int) -> list:
    ...
    new_tasks = []
    for task in tqdm(instructions, desc="Diversifying questions", unit="question"):
        ...
        prompt = (f"Please rewrite the following programming question to make it more suitable for a beginner Python student using the following method:\n{chosen_method}\n\n"
                  f"#Original Test#\n{task['instruction']}\n\n"
                  "Rewrite the question without any additional context or explanations. "
        ...
        if '#Rewritten Instruction#' in rewritten_instruction:
            rewritten_instruction = rewritten_instruction.split('#Rewritten Instruction#')[-1].strip()
        new_tasks.append({
...
def generate_diverse_dataset(
    ...
    sample_size=343  # Sample size for testing | 343 = max
):
